...
export function isCompactToolCallShorter(input: ToolCallInput): boolean {
...
export function assertTranscriptTheme(theme: VisageThemeDoc): {
	...
	if (out.userMessageBg.toLowerCase() === out.text.toLowerCase()) {
		throw new Error(
			"userMessageBg must differ from text for user bubble contrast",
		);
	}
	return out as {
		userMessageBg: string;
...
export function formatUsageSegments(
	...
	if (totals.cacheRead > 0)
		parts.push(`R${formatFooterTokens(totals.cacheRead)}`);
	if (totals.cacheWrite > 0)
		parts.push(`W${formatFooterTokens(totals.cacheWrite)}`);
	if (
...
export function resolveVisageThemeId(
