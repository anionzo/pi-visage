...
test("compact tool call is shorter than comfortable full chrome", () => {
	...
	assert.ok(
		full.includes("offset=") || full.includes("extra") || full.includes("("),
	);
});
...
test("summarizeToolArgs produces short path/command snippets", () => {
	assert.ok(
		summarizeToolArgs("read", { path: "/Users/me/proj/a.ts" }).includes("a.ts"),
	);
...
test("applyTranscriptTheme maps realistic user/assistant message fixtures via shipped dark theme", () => {
	...
	assert.notEqual(
		asstStyle.accent.toLowerCase(),
		asstStyle.muted.toLowerCase(),
	);
...
test("usage totals track cache read/write and Pi cache-hit formula", () => {
	...
	assert.ok(segs.some((s) => s.startsWith("↓")));
	assert.ok(
		segs.some((s) => s.startsWith("R")),
		`expected R cache read in ${segs.join(" ")}`,
	);
	assert.ok(
		segs.some((s) => s.startsWith("CH")),
		`expected CH% in ${segs.join(" ")}`,
	);
	assert.ok(
		!segs.some((s) => s.startsWith("W")),
		"zero cacheWrite should be omitted",
	);
...
test("formatUsageSegments omits cache fields when zero", () => {
	...
	assert.deepEqual(
		segs.filter(
			(s) => s.startsWith("R") || s.startsWith("W") || s.startsWith("CH"),
		),
		[],
	);
	assert.ok(segs.some((s) => s.startsWith("↑")));
...
test("skin.ts does not register dead customType message renderers for core roles", () => {
	const skin = fs.readFileSync(
		path.join(ROOT, "extensions", "skin.ts"),
		"utf8",
	);
	assert.ok(
	...
	assert.ok(
		skin.includes("cacheRead") || skin.includes("formatUsageSegments"),
		"footer must surface prompt cache",
	);
	assert.ok(
		skin.includes('cmd === "doctor"') || skin.includes("doctor"),
		"must expose /visage doctor",
	);
	assert.ok(
		skin.includes("setWidget") || skin.includes("applyContextWidget"),
		"must wire context widget API",
	);
...
test("pages stay sandboxed: no top-level import; unique ids", () => {
	...
	for (const file of files) {
		...
		assert.ok(
			...
			`${file} must not have top-level import`,
		);
...
test("extensions/ only contains factory entrypoints", () => {
	...
	for (const file of files) {
		...
		assert.ok(
			...
			`${file} must default-export a factory`,
		);
	}
});
