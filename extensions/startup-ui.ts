...
const PACKAGE_PAGES_DIR = path.join(PACKAGE_DIR, "..", "pages");
...
const CONFIG_PATH = path.join(os.homedir(), ".pi", "agent", "visage-ui.json");
...
function shortenPath(value: string, maxLength = 40): string {
	const home = os.homedir();
	let result = value;

	const startsWithHome =
		process.platform === "win32"
			? value.toLowerCase().startsWith(home.toLowerCase())
...
function padRightAnsi(text: string, targetWidth: number): string {
	const fitted = truncateToWidth(text, targetWidth, "");

	return fitted + " ".repeat(Math.max(0, targetWidth - visibleWidth(fitted)));
}

function centerAnsi(text: string, targetWidth: number): string {
	const fitted = truncateToWidth(text, targetWidth, "");

	const remaining = Math.max(0, targetWidth - visibleWidth(fitted));
...
function createFrame(
	...
	const frameWidth = Math.max(
		34,
		Math.min(options?.maxWidth ?? SETTINGS.maxHeaderWidth, terminalWidth),
	);
	...
	const accentBorder = (value: string) => theme.fg("borderAccent", value);
	...
	const top = title
		? accentBorder(
				`╭${title}${"─".repeat(
					Math.max(0, frameWidth - 2 - visibleWidth(title)),
				)}╮`,
			)
		: accentBorder(`╭${"─".repeat(innerWidth)}╮`);

	const body = content.map((line) => {
		return border("│") + padRightAnsi(line, innerWidth) + border("│");
	});
	...
	if (options?.footer) {
		...
		const available = Math.max(0, frameWidth - 2 - visibleWidth(footer));

		const left = Math.floor(available / 2);
		const right = available - left;

		bottom = accentBorder(
			`╰${"─".repeat(left)}` + footer + `${"─".repeat(right)}╯`,
		);
	} else {
		bottom = accentBorder(`╰${"─".repeat(innerWidth)}╯`);
	}

	const indent = " ".repeat(
		Math.max(0, Math.floor((terminalWidth - frameWidth) / 2)),
	);

	return [top, ...body, bottom].map((line) => {
		return truncateToWidth(indent + line, terminalWidth, "");
	});
}
...
function renderTwoColumns(
	...
	const frameWidth = Math.max(
		34,
		Math.min(options.maxWidth ?? SETTINGS.maxHeaderWidth, width),
	);
	...
	const rightWidth = Math.max(20, contentWidth - leftWidth - gapWidth - 2);
	...
	for (let index = 0; index < rowCount; index += 1) {
		rows.push(
			[
				" ",
				padRightAnsi(leftRows[index] ?? "", leftWidth),
				" ".repeat(gapWidth),
				truncateToWidth(rightRows[index] ?? "", rightWidth, ""),
			].join(""),
		);
	}

	return createFrame(theme, width, ["", ...rows, ""], options);
}
 ...
 */
function evaluateUiFile(filePath: string): StartupUiDefinition {
	...
	source = source.replace(/\bexport\s+default\s+/, "module.exports.default = ");

	if (
		!source.includes("module.exports.default") &&
		!source.includes("module.exports")
	) {
		throw new Error("Missing export default startup page definition.");
	}
	...
	validateUiDefinition(definition, filePath);
...
function validateUiDefinition(
	...
	if (
		...
		!/^[a-z0-9][a-z0-9_-]*$/i.test(definition.id)
	) {
		throw new Error(
	...
	const hasRenderer =
		...
		typeof definition.renderFull === "function" ||
		typeof definition.renderCompact === "function" ||
		typeof definition.renderTiny === "function";
...
function scanDirectory(dir: string): {
	...
	const map = new Map<string, StartupUiDefinition>();
	...
	for (const filePath of files) {
		try {
			const definition = evaluateUiFile(filePath);

			if (map.has(definition.id)) {
				throw new Error(`Duplicate startup page id: ${definition.id}`);
			}
...
function discoverStartupUis(): void {
	fs.mkdirSync(UI_DIRECTORY, {
		recursive: true,
	});

	// Package pages load first; user pages may override the same id.
	const packaged = scanDirectory(PACKAGE_PAGES_DIR);
	const homedir = scanDirectory(UI_DIRECTORY);

	const next = new Map<string, StartupUiDefinition>();

	for (const [id, def] of packaged.map) {
		next.set(id, def);
	}

	for (const [id, def] of homedir.map) {
		next.set(id, def);
	}

	loadedUis = next;
	loadErrors = [...packaged.errors, ...homedir.errors];

	if (loadedUis.size > 0 && !loadedUis.has(STATE.selectedId)) {
		STATE.selectedId = getSortedUis()[0].id;
...
function getSortedUis(): StartupUiDefinition[] {
	return [...loadedUis.values()].sort((a, b) => {
		const orderA = Number.isFinite(a.order) ? Number(a.order) : 1_000;

		const orderB = Number.isFinite(b.order) ? Number(b.order) : 1_000;

		return orderA - orderB || a.label.localeCompare(b.label);
	});
}

function getActiveUi(): StartupUiDefinition | undefined {
	return loadedUis.get(STATE.selectedId) ?? getSortedUis()[0];
}
...
function renderActiveUi(ctx: any, theme: Theme, width: number): string[] {
	const definition = getActiveUi();

	if (!definition) {
		return [
			theme.fg("warning", "No startup UI files found."),
			theme.fg("dim", `Directory: ${UI_DIRECTORY}`),
		];
	}

	const args = buildRenderArgs(definition, ctx, theme, width);

	if (width < 38 && typeof definition.renderTiny === "function") {
		return definition.renderTiny(args);
	}

	const useFull =
		STATE.layout === "full" ||
		(STATE.layout === "auto" && width >= SETTINGS.fullLayoutMinWidth);

	if (useFull && typeof definition.renderFull === "function") {
		return definition.renderFull(args);
	}

	if (!useFull && typeof definition.renderCompact === "function") {
	...
	if (typeof definition.renderFull === "function") {
		return definition.renderFull(args);
	}

	if (typeof definition.renderCompact === "function") {
		return definition.renderCompact(args);
	}
...
function createHeaderComponent(ctx: any, tui: any, theme: Theme) {
	requestRender = () => {
		tui.requestRender();
	};
	...
	return {
		render(width: number) {
			return renderActiveUi(ctx, theme, width);
		},
...
function createFooterComponent(
	...
	return {
		...
		render(width: number) {
			...
			const thinking = getThinking(currentPi);

			const leftParts = [
				theme.fg(
					"muted",
					`${formatPercent(usage.percent)}/${formatTokens(
						usage.contextWindow,
					)}`,
				),
			];

			if (SETTINGS.showWorkingDirectory) {
				leftParts.push(theme.fg("dim", shortenPath(ctx.cwd)));
			}

			const branch = footerData.getGitBranch?.() ?? null;

			if (branch) {
				leftParts.push(theme.fg("accent", `git:${branch}`));
			}

			const left = leftParts.join(theme.fg("dim", " · "));

			const right = [
				theme.fg("accent", model.id),
				theme.fg("dim", " · "),
				theme.fg("muted", `thinking ${thinking}`),
			].join("");
...
async function applyActiveUi(ctx: any): Promise<void> {
	currentContext = ctx;

	if (!STATE.enabled || ctx.mode !== "tui") {
		return;
	}
	...
	ctx.ui.setHeader((tui: any, theme: Theme) => {
		return createHeaderComponent(ctx, tui, theme);
	});

	if (SETTINGS.customFooter) {
		ctx.ui.setFooter((tui: any, theme: Theme, footerData: any) => {
			return createFooterComponent(ctx, tui, theme, footerData);
		});
	}

	if (typeof definition.workingIndicator === "function") {
		const indicator = definition.workingIndicator({
			pi: currentPi,
			ctx,
			theme: ctx.ui.theme,
			ui: UI_RUNTIME,
		...
		ctx.ui.setWorkingIndicator({
			...
			intervalMs: indicator.intervalMs ?? 140,
		});
	} else {
		ctx.ui.setWorkingIndicator({
			frames: [
	...
	ctx.ui.setTitle(
		`${definition.title ?? definition.label} · ${
			path.basename(ctx.cwd) || "workspace"
		}`,
	);

	ctx.ui.setStatus(
		EXTENSION_ID,
		ctx.ui.theme.fg("accent", `${definition.label} ready`),
	);
...
async function deactivateCurrentUi(ctx: any): Promise<void> {
	const definition = getActiveUi();
...
async function restoreBuiltInUi(ctx: any): Promise<void> {
...
function getCommandCompletions(prefix: string) {
	...
	const values = [
		...
		{
			value: "off",
			label: "off — disable custom startup UI",
		},

		{
			value: "reload",
			label: "reload — rescan startup page directories",
		},

		{
			value: "status",
			label: "status — show adapter status",
		},
	].filter((item) => {
		return item.value.startsWith(normalized);
	});
...
async function showStatus(ctx: any) {
	const active = getActiveUi();

	ctx.ui.notify(
		[
			`Visage UI: ${STATE.enabled ? "on" : "off"}`,
			`Current UI: ${active ? `${active.label} (${active.id})` : "none"}`,
			`Layout: ${STATE.layout}`,
			...
			"",
			...(loadErrors.length > 0 ? ["Load errors:", ...loadErrors] : []),
		].join("\n"),
		loadErrors.length > 0 ? "warning" : "info",
	);
}

async function chooseStartupUi(args: string, ctx: any) {
	discoverStartupUis();
	...
	let selected: string | "off" | undefined;

	if (requested) {
		if (requested === "off" || loadedUis.has(requested)) {
			selected = requested;
		} else {
			ctx.ui.notify(
				[
					...
					"",
					"Available pages:",
					...getSortedUis().map((item) => `${item.id} — ${item.label}`),
		...
		const current = STATE.enabled ? STATE.selectedId : "off";
		...
		const selectedText = await ctx.ui.select(
			"Select startup UI",
		...
		selected = entries.find((item) => item.text === selectedText)?.id;
	}
	...
	if (selected === "off") {
		...
		ctx.ui.notify("Restored Pi default startup UI.", "info");
...
export default function startupUiAdapter(pi: ExtensionAPI) {
	currentPi = pi;
