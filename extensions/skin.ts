...
function loadUiAdapterState(): UiAdapterState {
	try {
		...
		const raw = JSON.parse(fs.readFileSync(UI_CONFIG_PATH, "utf8")) as Record<
			string,
			unknown
		>;
		return {
...
function applyRecommendedSetup(
	...
	if (
		ctx?.mode === "tui" &&
		ctx?.hasUI &&
		typeof ctx.ui?.setTheme === "function"
	) {
		try {
...
function renderIdleLeft(theme: any, density: Density, width: number): string {
	const cmds =
		density === "compact" || width < 56
			? IDLE_SHORTCUTS_COMPACT
			: IDLE_SHORTCUTS_FULL;
	...
	ctx.ui.setFooter((tui: any, theme: any, footerData: any) => {
		...
		return {
			...
			render(width: number): string[] {
				...
				const gap = Math.max(
					1,
					width - visibleWidth(left) - visibleWidth(rightCore),
				);
...
function paintToolCall(
	...
	if (tool === "bash" && args?.timeout)
		extraBits.push(`timeout=${args.timeout}s`);
...
function paintToolResult(
	...
	if (options.isPartial) {
		return new Text(
			theme.fg("warning", density === "compact" ? "…" : "working…"),
			0,
			0,
		);
	}
		...
		handler: async (args, ctx) => {
			...
			if (cmd === "page") {
				...
				if (pageId === "off") {
					...
					reportLines(ctx, [
						"Startup page disabled (Pi default). Restart or /setStartUI off applied on next paint.",
					]);
					return;
				}
				saveUiAdapterState({
			...
			if (cmd === "density") {
				if (value !== "comfortable" && value !== "compact") {
					reportLines(
						ctx,
						["Usage: /visage density comfortable|compact"],
						"warning",
					);
					return;
				}
			...
			if (cmd === "theme") {
				const name =
					resolveVisageThemeId(value) ?? (value ? null : "visage-dark");
				if (!name) {
				...
				if (
					...
					reportLines(
						ctx,
						[`Theme → ${name} (not applied: non-TUI)`],
						"warning",
					);
				}
				return;
